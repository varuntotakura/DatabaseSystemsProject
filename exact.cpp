...
long match_searching(long t[], long eps_t, long s_0, long lmd_a, long lmd_d){
    int n = sizeof(t);
    long dp[99]; // stack<long> dp;
    long op[99]; // stack<long> op;
    for(int i=1; i<(n+1); i++){
        dp[i] = {};
        op[i] = {};
    }
...
long trace_back(long op[], int t[], long s[]){
    long s_0, eps_t, m_best;
    m_best = s;
    int n = sizeof(t);
    vector<int> M;
    int i = n;
...
long exact_repair(int t[], long lmd_a=10, long lmd_d=10, int interval_granularity=1, int start_point_granularity=1, int bias_d=1, int bias_s=3){
    long eps_list[];
    for(int i=0; i<sizeof(t)-1; i++){
        eps_list.push(t[i]-t[i-1]);
    }
    long eps_md = median(eps_list);
    long eps_t = round_to_granularity(eps_md, interval_granularity);
    long eps_t_traverse_range[];
    long eps_t_traversed[];
    float min_cost = 10e8;
    while(1){
        int d = 0;
        while(d == 0 || check_st_lb(d, eps_list, min_cost, lmd_d, eps_t)) && d < len(eps_list) && d < bias_d){
            ...
            while(s_0 <= t[d] + bias_s){
                long M, cost, m = match_searching(t, eps_t, s_0, lmd_a, lmd_d);
                if(cost < min_cost){
